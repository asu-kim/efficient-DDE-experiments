	...
	if [[ -d "$folder" ]]; then
		...
		for file in "$folder"/*.lf; do
			if [[ -f "$file" ]]; then
				...
				if [[ $? -eq 0 ]]; then
					mkdir -p "$bin_path/SOTA/$folder/$filename"

					mv "$bin_path/$filename" "$bin_path/SOTA/$folder/$filename/$filename"
					echo "Compiled $file successfully and moved to $bin_path/SOTA/$folder/$filename/$filename"
					pushd $bin_path/SOTA/$folder/$filename
...
if [ ! -f "'"$rti_abs_path"'" ]; then\
    ...
    exit 1\
					...
					./$filename
					popd
					echo "Ran $bin_path/SOTA/$folder/$filename/$filename."
				else
