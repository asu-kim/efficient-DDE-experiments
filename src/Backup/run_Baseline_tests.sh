		...
		for file in "$folder"/*.lf; do
			if [[ -f "$file" ]]; then
				...
				if [[ $? -eq 0 ]]; then
					...
					mkdir -p "../bin/Plain/$folder/$filename"

					mv "../bin/$filename" "../bin/Plain/$folder/$filename/$filename"
					echo "Compiled $file successfully and moved to bin/$filename"
					pushd ../bin/Plain/$folder/$filename
					./$filename
					popd
					echo "Ran ../bin/Plain/$folder/$filename/$filename."
				else
...
pushd ../Results
mkdir -p ./Plain
mv ./CycleWithDelay ./DistanceSensing ./SporadicSender ./Plain
popd
