...
bin_path="../bin"
lfc_path="../lingua-franca-HLA-like/bin/lfc-dev"
rti_relative_path="../lingua-franca-HLA-like/core/src/main/resources/lib/c/reactor-c/core/federated/RTI/build/RTI"
	...
	if [[ -d "$folder" ]]; then
		...
		for file in "$folder"/*.lf; do
			if [[ -f "$file" ]]; then
				...
				if [[ $? -eq 0 ]]; then
					mkdir -p "$bin_path/HLA_like/$folder/$filename"

					mv "$bin_path/$filename" "$bin_path/HLA_like/$folder/$filename/$filename"
					echo "Compiled $file successfully and moved to $bin_path/HLA_like/$folder/$filename/$filename"
					pushd $bin_path/HLA_like/$folder/$filename
...
if [ ! -f "'"$rti_abs_path"'" ]; then\
    ...
    exit 1\
					...
					./$filename
					popd
					echo "Ran $bin_path/HLA_like/$folder/$filename/$filename."
				else
...
mv $bin_path/HLA_like $results_path/
